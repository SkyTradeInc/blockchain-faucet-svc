...
const validateReCaptcha = (request, response, next) => {
  ...
  https.get(`https://www.google.com/recaptcha/api/siteverify?secret=${process.env.GOOGLE_CAPTCHA_SECRET}&response=${key}`, (google) => {
    let data = ''
    google.on('data', (chunk) => data += chunk.toString())
    google.on('end', () => {
      try {
        logger.debug(`(3/5) Passed reCaptcha`)
        return next();
      } catch (error) {
        logger.error(error)
...
router.get('/q', (request, response) => {
  return successResponse(response, 'Request limit', {limit: parseInt(process.env.REQUEST_LIMIT) || 3})
})
